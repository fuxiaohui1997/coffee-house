    ...
    <div class="content">
      ...
      <div v-if="$store.state.user.token">
        <van-tabs v-model="active"
          ...
          <van-tab title="全部订单">
            <!-- 订单渲染组件 -->
            <ordersitem :lists="search ()"></ordersitem>
          </van-tab>
      ...
      if (this.keyword == "") {
        ...
        return this.lists.filter(item => {
          return String(item.id).toLowerCase().indexOf(keyword) > -1 ||
            String(item.totalPrice).toLowerCase().indexOf(keyword) > -1 ||
            String(item.address.name).toLowerCase().indexOf(keyword) > -1 ||
            String(item.address.tel).toLowerCase().indexOf(keyword) > -1 ||
            item.goods.some(key => {
              return String(key.name).toLowerCase().indexOf(keyword) > -1
            })
        })
      }
    }
